...
**Canonical reference:** For the full operational workflow (all steps, prompts, and book chapters), see **`ForgeSpec_WORKFLOW.md`** at the repository root. If your idea is still vague, run **`prompts/PROMPT0_SPEC_DISCOVERY.md`** first (Specification Discovery); paste the resulting narrative into the PROJECT NARRATIVE section of PROMPT1_GENERATE_SPEC in Step 2 below.
...
# Step 2 — Run PROMPT1_GENERATE_SPEC.md

Paste the contents of `PROMPT1_GENERATE_SPEC.md` into the LLM and append the narrative.
...
# Step 3 — Run PROMPT2_VALIDATE_SPEC.md

Once the specification repository exists, run the validation prompt.
...
Tests and acceptance criteria should reflect the specification so that implementation is verified against the spec (see Chapter 12 and `ForgeSpec_WORKFLOW.md`).
