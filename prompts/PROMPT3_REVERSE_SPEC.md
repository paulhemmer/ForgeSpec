...
# Reverse-Engineering Spec Generator
...
description, or documentation) and derive a **formal spec repository**
that describes the system's architecture.

This process converts:

existing system → architecture model → spec repository
...
Do **not** emit the full `FILE:` spec repository tree until **emit** mode.

## When to emit the full repository

**Emit** the complete spec repository (OUTPUT + OUTPUT FORMAT) when **any** of these is true:
...
You must produce a **spec repository** using the following structure.
...
10. Complete spec repository contents
