...
For best results, run discovery in an environment where the assistant can read the repository (so it can use `prompts/specification_discovery_question_bank.md`), or paste that file into the chat along with the prompt.
...
Optional: use the **Controlled Implementation Loop** for slice-by-slice discipline—see `prompts/IMPLEMENTATION_EXECUTION.md` and prompts **Implementation Slice** / **Implementation Review** / **Commit Message** (`PROMPT5`–`PROMPT7` in `prompts/`). Optional Round 3 (Timing Realization): **Optimize** (`PROMPT8_OPTIMIZE.md`), see Chapter 12.
