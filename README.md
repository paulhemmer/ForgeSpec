...
<img src="assets/wide_header.png" width="1000">
...
This repository contains supporting material for *FORGESPEC: Architecture Before Code*, including prompts, examples, documentation, and the static landing page served at forgespec.io.
...
📘 Available in paperback, hardcover, and eBook on Amazon (6/2026)
...
• a grounded look at the craft and future of software engineering. 
...
5. For reader-facing prompt names and the prompt–chapter index, see **`book/ForgeSpec_Book_Outline.md`**
6. For optional bounded implementation discipline, see **`prompts/IMPLEMENTATION_EXECUTION.md`** (Controlled Implementation Loop)

Paths such as `docs/IMPLEMENTATION_PLAN.md` and `docs/TRACEABILITY.md` refer to files inside a **generated spec repository** you create with the prompts, not paths in this toolkit repository.
...
The **`prompts/`** directory ships the operational prompts (reader-facing names align with **Chapter 7** of the book):

| Capability | File | Description |
|------------|------|-------------|
| **Specification Discovery** | `prompts/PROMPT0_SPEC_DISCOVERY.md` | Interactive Q&A to refine a vague idea into a project narrative |
...
| **validate_spec** | `prompts/PROMPT2_VALIDATE_SPEC.md` | Deep architecture review; detect design flaws and drift |
...
| **Implementation Slice** | `prompts/PROMPT5_IMPLEMENTATION_SLICE.md` | Bounded slice brief from the implementation plan (Controlled Implementation Loop) |
| **Implementation Review** | `prompts/PROMPT6_IMPLEMENTATION_REVIEW.md` | Adversarial gate on a slice’s implementation delta |
| **Commit Message** | `prompts/PROMPT7_COMMIT_MESSAGE.md` | Structured commit message listing IDs and behavioral impact |
| **Reorient** | `prompts/PROMPT_REORIENT.md` | Re-read plan and traceability; short status report before more work |
| **Optimize** | `prompts/PROMPT8_OPTIMIZE.md` | Optional Round 3: measurement‑backed performance tuning after correctness |

Supporting files include **`prompts/IMPLEMENTATION_EXECUTION.md`** (Controlled Implementation Loop), **`prompts/specification_discovery_question_bank.md`**, and **`prompts/design_review_template.md`**.

Together these support **both new systems and legacy systems**, plus optional slice‑by‑slice implementation discipline.
...
Typical structure (will vary by your own system scope):
...
docs/
    ...
    GLOSSARY.md
    design_reviews/
        (Validate Spec artifacts — save new reviews here)

prompts/
    kickoff_prompt.md

design_reviews/
    (empty placeholder from Generate Spec — you may remove after creating docs/design_reviews/)
```

The `spec/` directory defines the **canonical architecture**.

The `docs/` directory contains explanatory and operational documentation. **Validate Spec** (`prompts/PROMPT2_VALIDATE_SPEC.md`) expects design‑review artifacts under **`docs/design_reviews/`**; use that as the canonical location even if **Generate Spec** also emitted an empty top‑level `design_reviews/` folder.

The generated **`prompts/kickoff_prompt.md`** is the ongoing implementation contract for agents working in that repository.
...
Optional first step: **`prompts/PROMPT0_SPEC_DISCOVERY.md`** (Specification Discovery) when the problem narrative is still unclear; paste its output into **Generate Spec**.
...
PROMPT2_VALIDATE_SPEC (validate_spec)
        ↓
implementation (optionally: Controlled Implementation Loop — `prompts/IMPLEMENTATION_EXECUTION.md`, PROMPT5–PROMPT7)
```
...
Think of the book as the **methodology**, and the repository as the **toolkit**. The book also takes a grounded look at the craft and future of software engineering as a profession.
